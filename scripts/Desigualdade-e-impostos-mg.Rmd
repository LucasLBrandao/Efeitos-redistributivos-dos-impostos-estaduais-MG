...
3 - Renda total mensal domiciliar per capita - deduções mensal domiciliar per capita + IPVA
...
## Cálculo do total gasto  anualmente com IPVA por unidade de consumo
...
## Cálculo do total gasto anualmente com ICMS por unidade de consumo
...
prod_UC_despesa_coletiva <- despesa_coletiva %>%
  # Filtro para despesas apenas referentes à gastos
        ...
        mutate(V9011 = ifelse(QUADRO  %in% c(10,19),V9011,1),
               V9011 = ifelse(is.na(V9011)|V9011 == 0,1,V9011),
...
### Cálculo da alíquota efetiva do ICMS para cada produto da TRU (Problemático)

Para cálculo da alíquota efetiva seguiremos a metodologia feita por Afonso et al. (2004).
Afonso parte do pressuposto de que os insumos dos produtos não são tributados, pelo icms se tratar de um Imposto sobre valor adicionado. assim a base de cálculo seriam os componentes da demanda final. para manter esse pressuposto é importante utilizarmos os produtos presentes na Matriz Insumo produtoque apresentam maior participação do consumo das famílias na demanda final.
...
tru_consumo <- tru_consumo %>%
  rowwise() %>% 
  mutate(perc_familias = consumo_familias/(`Exportação\nde bens e\nserviços (1)` + `Consumo\ndo governo` + `Consumo\ndas\n ISFLSF` + consumo_familias)) %>% 
        #select(COD_PROD_TRU,consumo_familias,perc_familias) %>%
        filter(COD_PROD_TRU != "Total",!is.na(COD_PROD_TRU))


tru_consumo_selecionado <- tru_consumo %>% filter(perc_familias > 0.4)
```
...
tru_consumo_icms <- tru_consumo_selecionado %>% 
...
tradutor_icms <- tradutor_icms %>% mutate(`Produto POF4` = substr(`Produto POF`,1,nchar(`Produto POF`)-2)) 
...
## Criação de coluna com renda após gasto com IPVA e ICMS
...
population_totals_mg <- 
    data.frame(
        pos_estrato = unique( pos_estrato_mg$pos_estrato ) , 
        Freq = unique( pos_estrato_mg$TOTAL_PESSOAS_REFERENCIA ) 
    )
...
## Divisão das famílias em classes de renda
...
#k <- round(
 # c(0,svyquantile(~RENDA_TOTAL_MENSAL_PC, pof_design_mg,seq(0.1,0.9,0.1)),Inf)
  #) 

k2 <- round(c(0,svyquantile(~RENDA_TOTAL_MENSAL_PC, pof_design_mg,seq(0.01,0.99,0.01)),Inf))
...
familias_classes_renda_design <- 
  transform(
    ...
    classes_renda = cut(RENDA_TOTAL_MENSAL_PC,
                        ...
                        include.lowest = T,
                        ordered_result = T,
                        labels = c(paste("Até", k[2]),
                                   paste(paste(rep("entre",8),k[2:9]),
                                         paste(rep("e",8),k[3:10])),
                                   paste("Mais que", k[10]))),
...
saveRDS(pof_impostos_srvyr_design, file = "POF_mg_design_icms_ipva.rds")
...
## Comparando o gini nos estágios de renda
...
comparação_gini_mg <- moradores %>% select(UF,ESTRATO_POF,TIPO_SITUACAO_REG,COD_UPA,NUM_DOM,NUM_UC,V0306,V0430,V0404,V0405,INSTRUCAO,ANOS_ESTUDO,PESO_FINAL) %>% 
 mutate(FAMILIA = paste(COD_UPA,NUM_DOM,NUM_UC),
        ...
        V0405 = case_when(V0405 == 1 ~ "Branca",
                          ...
                          TRUE ~ "Vazio"),
        INSTRUCAO_short = case_when(INSTRUCAO == 1 ~ "Sem intrução",
                                    INSTRUCAO <= 4 ~ "Até Fundamental completo",
                                    INSTRUCAO <= 6 ~ "Até Médio completo",
...
saveRDS(pof_design_moradores,"POF_mg_design_moradores_icms_ipva.rds")
...
svylorenz( ~RENDA_TOTAL_ANUAL, pof_design_moradores_prep, quantiles = seq( 0, 1, .05 ), na.rm = TRUE )

svylorenz( ~renda_pos_ipva, pof_design_moradores_prep, quantiles = seq( 0, 1, .05 ), na.rm = TRUE )
png()
svylorenz( ~renda_pos_icms, pof_design_moradores_prep, quantiles = seq( 0, 1, .05 ), na.rm = TRUE )
```
...
con_index_ipva <- ci(pof_design_moradores$variables$gasto_veiculos_anualizado, pof_design_moradores$variables$renda_pos_ipva,wt=pof_design_moradores$variables$peso, type = c("CI"))
...
## Despesas com impostos nos diferentes grupos sociais

Por raça

```{r}
tabela_raca_gasto <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(V0405) %>%
  summarise(renda_total_media = survey_mean(RENDA_TOTAL_ANUAL,w = peso,na.rm = T),
            despesa_ipva_media = survey_mean(gasto_veiculos_anualizado,w = peso,na.rm = T),
...
tabela_raca_gasto <- tabela_raca_gasto %>% select(-c(renda_total_media_se,
                                                         despesa_ipva_media_se,
                                                         despesa_icms_media_se,
                                                         despesa_total_media_se)) %>% 
  filter(V0405 != "Não declarado")

kbl(tabela_raca_gasto,col.names = c("Raça",
                                      "Renda total média",
                                      "IPVA",
                                      "ICMS",
                                      "Impostos estaduais",
      ...
      align = c("lcccccc"),
      booktabs = T,
      caption = "Carga tributária entre as raças",
      format.args = list(big.mark = " ",scientific = FALSE)) %>% 
      kable_styling(font_size = 7,
                latex_options = c("striped", "scale_down"),
                full_width = T) %>% 
      add_header_above(c(" " = 2, "Despesa média" = 3, "% da Renda total média"))
...
tabela_sexo_gasto <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(V0404) %>%
  summarise(renda_total_media = survey_mean(RENDA_TOTAL_ANUAL,w = peso,na.rm = T),
            despesa_ipva_media = survey_mean(gasto_veiculos_anualizado,w = peso,na.rm = T),
...
tabela_sexo_gasto <- tabela_sexo_gasto %>% select(-c(renda_total_media_se,
                                                         despesa_ipva_media_se,
                                                         despesa_icms_media_se,
                                                         despesa_total_media_se))

kbl(tabela_sexo_gasto,col.names = c("Sexo",
                                      "Renda total média",
                                      "IPVA",
                                      "ICMS",
                                      "Impostos estaduais",
...
```{r}
tabela_escola_gasto <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(INSTRUCAO_short) %>%
  summarise(renda_total_media = survey_mean(RENDA_TOTAL_ANUAL,w = peso,na.rm = T),
            despesa_ipva_media = survey_mean(gasto_veiculos_anualizado,w = peso,na.rm = T),
            despesa_icms_media = survey_mean(gasto_icms,w = peso, na.rm = T),
            despesa_total_media = survey_mean(gasto_impostos,w = peso,na.rm = T)) %>% 
...
tabela_escola_gasto <- tabela_escola_gasto %>% select(-c(renda_total_media_se,
                                                         despesa_ipva_media_se,
                                                         despesa_icms_media_se,
                                                         despesa_total_media_se)) %>% 
  filter(INSTRUCAO_short != "Não considerado")

kbl(tabela_escola_gasto,col.names = c("Escolaridade",
                                      "Renda total média",
                                      "IPVA",
                                      "ICMS",
                                      "Impostos estaduais",
...
```{r}
tabela_sexo_raca_gasto <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(sexo_raca) %>%
  summarise(renda_total_media = survey_mean(RENDA_TOTAL_ANUAL,w = peso,na.rm = T),
            despesa_ipva_media = survey_mean(gasto_veiculos_anualizado,w = peso,na.rm = T),
...
tabela_sexo_raca_gasto <- tabela_sexo_raca_gasto %>% select(-c(renda_total_media_se,
                                                         despesa_ipva_media_se,
                                                         despesa_icms_media_se,
                                                         despesa_total_media_se)) %>% 
  filter(!grepl("Não declarado",sexo_raca))

kbl(tabela_sexo_raca_gasto,col.names = c("Sexo e raça",
                                      "Renda total média",
                                      "IPVA",
                                      "ICMS",
                                      "Impostos estaduais",
...
## Participação no gasto tributário

Por raça

```{r}
tabela_raca_participacao <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(V0405) %>% 
  summarise(renda_total = survey_total(RENDA_TOTAL_ANUAL,na.rm = T),
            despesa_ipva_total = survey_total(gasto_veiculos_anualizado,na.rm= T),
...
tabela_raca_participacao <- tabela_raca_participacao %>% select(-c(renda_total,
                                                         despesa_ipva_total,
                                                         despesa_icms_total,
                                                         despesa_total,
                                                         renda_total_se,
...
kbl(tabela_raca_participacao,col.names = c("Raça",
                                      "Renda total",
                                      "Gasto com IPVA",
                                      "Gasto com ICMS",
                                      "Gasto com impostos"),
      align = c("lcccc"),
      booktabs = T,
      caption = "Participação no gasto com impostos entre as raças",
      format.args = list(big.mark = " ",scientific = FALSE)) %>% 
      kable_styling(font_size = 7,
                latex_options = c("striped", "scale_down"),
                full_width = T) %>% 
...
tabela_sexo_participacao <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(V0404) %>% 
  summarise(renda_total = survey_total(RENDA_TOTAL_ANUAL,na.rm = T),
            despesa_ipva_total = survey_total(gasto_veiculos_anualizado,na.rm= T),
...
tabela_sexo_participacao <- tabela_sexo_participacao %>% select(-c(renda_total,
                                                         despesa_ipva_total,
                                                         despesa_icms_total,
                                                         despesa_total,
                                                         renda_total_se,
...
kbl(tabela_sexo_participacao,col.names = c("Sexo",
                                      "Renda total",
                                      "Gasto com IPVA",
                                      "Gasto com ICMS",
                                      "Gasto com impostos"),
      align = c("lcccc"),
      booktabs = T,
      caption = "Participação no gasto com impostos entre os sexos",
      format.args = list(big.mark = " ",scientific = FALSE)) %>% 
      kable_styling(font_size = 7,
                latex_options = c("striped", "scale_down"),
                full_width = T) %>% 
...
tabela_escola_participacao <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(INSTRUCAO_short) %>% 
  summarise(renda_total = survey_total(RENDA_TOTAL_ANUAL,na.rm = T),
            despesa_ipva_total = survey_total(gasto_veiculos_anualizado,na.rm= T),
...
tabela_escola_participacao <- tabela_escola_participacao %>% select(-c(renda_total,
                                                         despesa_ipva_total,
                                                         despesa_icms_total,
                                                         despesa_total,
                                                         renda_total_se,
...
kbl(tabela_escola_participacao,col.names = c("Escolaridade",
                                      "Renda total",
                                      "Gasto com IPVA",
                                      "Gasto com ICMS",
                                      "Gasto com impostos"),
      align = c("lcccc"),
      booktabs = T,
      caption = "Participação no gasto com impostos entre diferentes escolaridades",
      format.args = list(big.mark = " ",scientific = FALSE)) %>% 
      kable_styling(font_size = 7,
                latex_options = c("striped", "scale_down"),
                full_width = T) %>% 
...
```{r}
tabela_sexo_raca_participacao <- pof_design_moradores %>% 
  as_survey() %>% 
  group_by(sexo_raca) %>% 
  summarise(renda_total = survey_total(RENDA_TOTAL_ANUAL,na.rm = T),
            despesa_ipva_total = survey_total(gasto_veiculos_anualizado,na.rm= T),
...
tabela_sexo_raca_participacao <- tabela_sexo_raca_participacao %>% select(-c(renda_total,
                                                         despesa_ipva_total,
                                                         despesa_icms_total,
                                                         despesa_total,
                                                         renda_total_se,
...
kbl(tabela_sexo_raca_participacao,col.names = c("Sexo e raça",
                                      "Renda total",
                                      "Gasto com IPVA",
                                      "Gasto com ICMS",
                                      "Gasto com impostos"),
